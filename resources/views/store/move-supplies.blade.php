...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Передача товаров из ' . $storeName) }}
        </h2>
    </x-slot>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="container">
                    ...
                    @else
                        <form action="{{ route('stores.store-move-supplies', $storeId) }}" method="POST">
                            @csrf
                            @method('POST')
                            ...
                            <div class="py-2">
                                <x-input-label class="py-2" for="store" :value="__('Передать в')"/>
                                <select name="store" id="store"
                                        class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full">
                                    @foreach($stores as $id => $store)
                                        <option value="{{ $id }}" {{ (int) old('store') === $id ? 'selected' : '' }}>{{ $store }}</option>
                                    @endforeach
                                </select>
                                <x-input-error :messages="$errors->get('store')" class="mt-2"/>
                            </div>
                            <x-primary-button class="mt-4">{{ __('Передать товары') }}</x-primary-button>
                        </form>
                    @endif
                </div>
            </div>
